...
impl Server {
    pub fn new(ip_address: String, port: String, protocol: String, server_name:String, key: String) -> Server {
            ...
            TCP => {
                match thread::Builder::new().name(server_name).spawn(move || {
                    match TcpStream::connect(addr) {
                        Ok(mut server) => {
                            Server::read_response_tcp(&mut server, &tx_response,&key);
                            loop {
                                match &rx.try_recv() {
                                    Ok(message) => {
                                        ...
                                        let key_gen = Key::from_slice(key.as_bytes());
                                ...
                                Ok(_) => loop {
                                    match &rx.try_recv() {
                                        Ok(message) => {
                                            ...
                                            Server::read_response_udp(&mut server, &tx_response, &key);
                                        }
                                        Err(TryRecvError::Disconnected) => {
                                            println!("Terminating.");
                                            break;
